...
//[ALLOCATE MEMORY PTR FUNCTION]
typedef void (*ptr_allocate_memory)(_processInfo pi, long long int* address, int amount);
//[READ FUNCTIONS]
...
void func_allocate_memory(_processInfo pi, long long int* address, int amount);
...
//[INITIALIZE ALLOCATE MEMORY FUNCTION]
void func_allocate_memory(_processInfo pi, long long int* address, int amount)
{
	if (pi.state == found) *address = VirtualAllocEx(pi.hproc, NULL, amount, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
}